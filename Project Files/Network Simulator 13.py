...
class CreatePacketPath:
    ...
    def CreatePackets(self):
        ...
        for packet in packets: # must be after for loop above
            #print(packet.ReturnNodeFrom(), packet.ReturnNodeTo())
            ...
            for node_to in self.adjacency_list[node_from]:
                if node_from.returnName() == "pc":
                    ...
                    if node_to.returnName() in self.connections:
                        packets.append(Packets(node_from, node_to, self.colour))
                    elif node_to.returnName() in ("switch", "hub"):
                        self.temp_packets.append([self.route_number, node_from, node_to])

                elif node_to.returnName() in self.connections:
                    for temp_list in self.temp_packets:
                        if temp_list[0] == self.route_number:
                            packets.append(Packets(temp_list[1], temp_list[2], self.colour))
                ...
                if node_to.returnName() in ("file server", "print server", "e-mail server", "multimedia server", "web cache"):
                    packets.append(Packets(node_to, node_from, self.colour))
                    packets.append(Packets(node_from, self.root_node, self.colour))

                if node_to in self.adjacency_list:
                    self.RecursiveDepthSearch(node_to)

        except TypeError: # if no property is selected
            pass
...
class Packets:
    def __init__(self, node_from, node_to, colour):
    ...
    def CalculateHierarchy(self):
        if self.node_from.returnName() == "pc":
            ...
            if self.node_to.returnName() == "printer": # should check if connected to print server. If so - go there then printer
                if any(packet.ReturnNodeFrom().returnName() == "print server" for packet in packets):
                    self.hierarchy_position = 6
                else:
                    self.hierarchy_position = 2
            elif self.node_to.returnName() == "pc":
                self.hierarchy_position = 5
            else:
                self.hierarchy_position = 2
        elif self.node_from.returnName() == "switch":
            if self.node_to.returnName() == "router":
                if any(packet.ReturnNodeFrom().returnName() == "web cache" for packet in packets):
                    self.hierarchy_position = 6
                elif self.node_to.returnName() == "pc":
                    self.hierarchy_position = 5
                else:
        ...
        #elif self.node_from.returnName() in ("file server", "print server", "e-mail server", "multimedia server", "web cache"):
            #self.hierarchy_position = 3
        else: # file server
            self.hierarchy_position = 4
        #else: can't connect from ie printer
            #self.hierarchy_position = 5
