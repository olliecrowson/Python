                    ...
                    if last_release - last_click < 100:  # track if dragged or clicked
                        ...
                        if self.x < 85:  # to delete node
                            ...
                            if adj_activated:
                                ...
                                # packet.DeleteNode ??
                        elif NodeAdjacencyList.ReturnState():
                            for packet in packets:
...
class PC(Node):
    def __init__(self):
        ...
        self.PropertyConnections = [{"Word processing": "printer"}, {"Internet browsing": "router"},
...
class AdjacencyList(object):
    ...
    def AddToDict(self, NodeFrom, NodeTo, x1, y1):
        ...
        if NodeFrom.returnName() == "pc":
            self.Node_Connections.append([NodeFrom, NodeTo])
        elif NodeTo.returnName() == "pc":
            self.Node_Connections.append([NodeTo, NodeFrom])
        else:
            self.temporary_nodes.append([NodeFrom, NodeTo])

        for temp_nodes in self.temporary_nodes:
            for node_list in self.Node_Connections:
                original_list = len(node_list)
                a = 0
                while a < original_list:
                    if temp_nodes[0] == node_list[a]:
                        node_list[a] = [node_list[a]]
                        node_list[a].append(temp_nodes[1])
                        self.temporary_nodes.remove(temp_nodes)
                        a = 100
                    elif temp_nodes[1] == node_list[a]:
                        node_list[a] = [node_list[a]]
                        node_list[a].append(temp_nodes[0])

                        #node_list.insert(a+1, (temp_nodes[0]))
                        self.temporary_nodes.remove(temp_nodes)
    ...
    def DisplayCables(self):
        ...
        for node_list in self.Node_Connections:
            for index, node in enumerate(node_list):
                try:
                    x1, y1 = node.returnPosition()
                    x2, y2 = node_list[index+1].returnPosition()
                    pygame.draw.aaline(screen, (0, 0, 255), (x1 + 20, y1 + 20), (x2 + 20, y2 + 20), 2)
                except IndexError:
                    pass
                except AttributeError:
                    for index2, nodes in enumerate(node_list[index+1]):
                        x1, y1 = nodes.returnPosition()
                        try:
                            x2, y2 = node_list[index+1][index2+1].returnPosition()
                            pygame.draw.aaline(screen, (0, 0, 255), (x1 + 20, y1 + 20), (x2 + 20, y2 + 20), 2)
        ...
        for key in keys:
            for index, node in enumerate(self.Node_Connections[key]):
                if index != 0:
                    print(node, index)
                    x1, y1 = node.returnPosition()
                    print(x1, y1)
                    x, y = self.Node_Connections[key][index-1].returnPosition()
                    pygame.draw.aaline(screen, (0, 0, 255), (x1 + 20, y1 + 20), (x + 20, y + 20), 2)
                else:
                    x1, y1 = self.Node_Connections[key][0].returnPosition()
                    print(x1, y1)
...
class CreatePacketPath:
    def __init__(self):
        self.hierarchy = ["pc", "switch", "hub"]

    def CalculatePath(self):
        self.PacketRoute = {}
        self.newroute = {}
        adjacency_list = NodeAdjacencyList.ReturnDict()
        # adjacency_list = [(k, adjacency_list[k].returnName()) for k in self.hierarchy if k in adjacency_list[]]
        # for node_from, node_to in adjacency_list.items():
        # for all (node_from.returnName() == "pc"):

        for node_object in adjacency_list:
            if node_object.returnName() == "pc":
                self.PacketRoute[node_object] = adjacency_list[node_object]
                #for node_to in adjacency_list:
                    #pass

        for node in self.hierarchy:
            for node_object in adjacency_list:
                if node == node_object.returnName():  # for node_to
                    self.PacketRoute[node_object] = (adjacency_list[node_object])
                    for node_to in (adjacency_list).itervalues():
                        for node in node_to:
                            if node in adjacency_list:
                                print("HELLO")
                                print(node)
                                self.PacketRoute[node] = (adjacency_list[node])
                            #print(self.PacketRoute)
        '''
        for packet in packets:
            if packet.returnNodeFrom() == "pc":
                self.newroute[packet.returnNodeFrom()] = (packet.returnNodeTo())
            elif not any(packet.returnNodeFrom() == "pc" for packet in packets):
                pass # with file server and current program running on PC
        '''
        self.CreatePackets()
    ...
    def CreatePackets(self):
        temp_packets_list = []
        del packets[:]
        for node_from in self.PacketRoute:
            for node_to in self.PacketRoute[node_from]:
                packets.append(Packets(node_from, node_to))

        for packet in packets:
            packet.CalculateGradient()
            packet.CalculateHierarchy()
...
class Packets:
    ...
    def currentlyMoving(self):
        self.currently_moving = True

    def returnCurrentlyMoving(self):
        return self.currently_moving

    def blitToScreen(self):
        # screen.fill((250,250,250)) # this doesnt work
        ...
        pygame.display.update()
        # clock.tick(1)
        # while (self.x, self.y) != (self.x2, self.y2):
        # pass
...
def ControlPackets():
    for packet in packets:
        ...
        if packet.returnHierarchy() == (1, False):
            packet.currentlyMoving()
            # completed_paths.append(packet)
            # packets.remove(packet)
        elif any(packet.returnHierarchy()[0] == 1 for packet in packets):
            if packet.returnHierarchy() == (2, False):
                if not any(packet.returnHierarchy() == (1, False) for packet in packets):
                    packet.currentlyMoving()
            else:
                if not any(packet.returnHierarchy() == (2, False) for packet in packets):
                    packet.currentlyMoving()
        if not any(packet.returnHierarchy()[1] == False for packet in packets):
            for packet in packets:
                packet.UpdateCycleStatus()
                # if all( packet.returnHierarchy() == (1, True) is True for packet in packets):
                # print("H")

    '''
    for index, packet in enumerate(packets): #should be if?
        cycle = packet.ReturnCycleStatus()
        if not cycle:
            try:
                packets[index+1].UpdateCycleStatus(True)
                print(packets[index+1].ReturnNodeFrom().returnName())
            except IndexError:
                for packet in packets:
                    packet.UpdateCycleStatus(True) # whole cycle completed. Reset to default values.
                screen.fill((250,250,250))
        else:
            if packet.ReturnNodeFrom().returnName() == "pc":
                packet.blitToScreen()
                #packet.UpdateCycleStatus(False)
    '''
...
def callBlitToScreen():
    ...
    if Server.returnActivated():  # must be separate to similar statement above so that the servers are above the cables
        Server.BlitServer()
    for packet in packets:
        if packet.returnCurrentlyMoving():
            packet.blitToScreen()
    pygame.display.flip()
