        ...
        while not done:
            ...
            while not mouse_down and not exit_cable:
                ...
                if mouse_down and node_to != None:
                    if node_to.returnActivated() == False:
                        mouse_down = False
...
class PC(Node):
    def __init__(self, CompatibleNodes):
        super(PC, self).__init__()
        self.CompatibleNodes = [CompatibleNodes] #"pc", "router", "switch", "hub", "printer"]

    def CheckCompatibility(self, node_to):
        if node_to in self.CompatibleNodes:
            return True
        else:
            return False
...
PCs = [Node((10,15),"pc.png","pc",1) for x in range (NUMBERPCS)]
Routers = [Node((10,60),"router.png","router",1) for x in range (NUMBERROUTERS)]
Switches = [Node((10,105),"switch.png","switch",1) for x in range (NUMBERSWITCHES)]
Hubs = [Node((10,170),"hub.png","hub",1) for x in range (NUMBERHUBS)]
Printers = [Node((10,215),"printer.png","printer",1) for x in range (NUMBERPRINTERS)]
