    ...
    def CalculatePath(self):
        ...
        for node in self.hierarchy:
            for node_object in adjacency_list:
                if node == node_object.returnName():
...
class Packets:
    ...
    def CalculateGradient(self):
        ...
        try:
            self.line_gradient = float(self.y2 - self.y1) / (self.x2 - self.x1)
        except ZeroDivisionError:
            self.line_gradient = 0

    def CalculateStartPosition(self):
        print(self.line_gradient)

    def blitToScreen(self):
        self.CalculateStartPosition()
        #self.x += (self.x * 0.2*(self.line_gradient))
        #self.y += (self.y * 0.7 * (abs(self.line_gradient)))
...
def runProgram():
    ...
    if NodeAdjacencyList.ReturnState():
