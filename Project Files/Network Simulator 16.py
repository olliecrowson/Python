import pygame, random, os, pickle, Database, time  # import libraries
pygame.init()  # initialise pygame
pygame.font.init()  # initialise pygame font creator
pygame.display.set_caption("Network Simulator")  # set program caption
clock = pygame.time.Clock()


os.environ['SDL_VIDEO_WINDOW_POS'] = "%d,%d" % (0, 25)  # set the position of the program window

FPS = 60  # set program frame constant
screenInfo = pygame.display.Info()  # get monitor screen dimensions
screen = pygame.display.set_mode((screenInfo.current_w, screenInfo.current_h - 64))
screen.fill((250, 250, 250))  # set colour of screen to off white

# create list of different RGB packet colours
...
class Node(object):  # parent class of devices
    ...
    def hoveringOver(self): # check if hovering over node
        ...
        if (mouse_x >= self.x) and (mouse_x <= self.x + 40):  # if touching in x-axis
            if (mouse_y > self.y) and (mouse_y < self.y + 40): # if touching in y-axis
                self.touching = True
                return True
    ...
    def dragDrop(self, node): # drag and drop GUI function
        ...
        for event in ev:
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:  # if left mouse button pressed
                last_click = pygame.time.get_ticks()
                self.mouse_down = True
                nodes.append(nodes.pop(nodes.index(node)))
                mouse_x, mouse_y = pygame.mouse.get_pos()
                # offset difference between node and cursor's original position
        ...
        while self.mouse_down:
            for event in pygame.event.get():
                if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                    ...
                    if last_release - last_click < 100:  # if mouse clicked then released
                        if self.activated:  # if node been dragged on canvas
                            if self.properties_window_activated:
                                self.selectProperty()  # display window to select running apps
                            else:
                                self.createPropertiesWindow()  # create window
                    else:
                        screen.fill((250, 250, 250))
                        self.activated = True
                        if self.x < 100:  # behind sidebar - will delete node
                            self.x, self.y = self.default_x, self.default_y
                            self.activated = False
                            adj_activated = NodeAdjacencyList.returnState()
                            if adj_activated:  # if matrix activated
                                NodeAdjacencyList.removeFromDict(node)
                                PacketPath.createPackets()

                        elif PacketPath.returnActivated():  # remove node from packets
                            for packet in packets:
                                packet.calculateGradient()

                elif self.mouse_down and self.movable:  # move node to new position
                    mouse_x, mouse_y = (pygame.mouse.get_pos())
    ...
    def createPropertiesWindow(self): # create window for selection of running apps./servers and to display name/ icon
        self.current = True
        ...
        y = 90
        callBlitToScreen()
        self.window_surface = pygame.Surface((400, 500))  # create new surface
        self.window_surface.fill((255, 255, 255))  # white fill
        pygame.draw.rect(screen, (0, 0, 0), (500, 50, 400, 500), 8)  # draw box outline
        text = pygame.font.SysFont('Comic Sans MS', 25)  # initialise fonts
        node_name_text = text.render(self.name, True, (0, 0, 0))
        text = pygame.font.SysFont('Comic Sans MS', 20)
        words = [word.split(' ') for word in self.text.splitlines()]  # 2D array where each item is a list of words.
        for line in words:  # pygame can't render line breaks
            for word in line:
                node_properties_text = text.render(word, True, (0, 0, 0))
                word_width, word_height = node_properties_text.get_size()  # get size of text
                self.window_surface.blit(node_properties_text, (x, y))  # draw text to window
                if word:  # if not blank line
                    x = x + (word_width + 5)  # add the width of the word to x
            x = 50
            y = y + (word_height + 9)  # add height of word to y
            if not line[0]:  # do not create a button next to title
                pygame.draw.circle(self.window_surface, (0, 0, 0), (x - 16, y - 22), 12, 1)  # draw checkboxes
                self.checkbox_coordinates.append([[x - 16, y - 22], line])  # add checkboxes to list
                line.pop(0)  # remove blank line from words list

        self.window_surface.blit(self.default_image, (50, 10))  # draw node icon to window
        self.window_surface.blit(node_name_text, (50, 50))  # write node name to window
        screen.blit(self.window_surface, (500, 50))  # must be after blitting image to surface
        pygame.display.update()
        self.selectProperty()  # for checkbox selection

    def selectProperty(self): # get selection of running apps. from checkbox
        done = False
        pygame.draw.rect(screen, (0, 0, 0), (500, 50, 400, 500), 8)  # draw box window
            ...
            for event in pygame.event.get():
                for coordinate in self.checkbox_coordinates:
                    ...
                    if event.type == pygame.MOUSEBUTTONUP:  # if mouse clicked
                        # check if cursor touching checkbox
                        if (x <= (coordinate[0][0] + 511)) and ((x >= coordinate[0][0] + 488)):
                            if (y <= (coordinate[0][1] + 61)) and ((y >= coordinate[0][1] + 38)):
                                selection = coordinate[1]  # text (i.e. [Word, processing])
                                selection = ' '.join(str(x) for x in selection)  # joins words together

                                if selection in self.properties:  # if already selected
                                    self.properties.remove(selection)  # deselect checkbox and remove property
                                    pygame.draw.rect(self.window_surface, (255, 255, 255), (coordinate[0][0] - 12, coordinate[0][1] - 12, 24, 24)) # draw blank rectangle over ticked one
                                    pygame.draw.circle(self.window_surface, (0, 0, 0), (coordinate[0][0], coordinate[0][1]), 12, 1)  # draw blank circle over rectangle
                                    for server in Server_drives:
                                        if server.returnActivated():
                                            server.RemoveReduntantCables()  # remove drive from server

                                else:  # if not been selected already
                                    self.window_surface.blit(pygame.image.load("tick.png"), (coordinate[0][0] - 9, coordinate[0][1] - 7))  # add tick to circle
                                    self.properties.append(selection)  # add selection to list

                        elif (x < 500) or (x > 900) or (y < 50) or (y > 550):  # to quit selection box
                            done = True

                        screen.blit(self.window_surface, (500, 50))  # draw new checkbox selections to screen
                        pygame.display.update()
...
class AdjacencyList(object): # 'object' since it is a parent class
    def __init__(self):
    ...
    def addToDict(self, node_from, node_to): # add route to dict
        self.activated = True
        if node_from.returnName() == "pc": # connections from PCs takes ultimate priority
            if node_from in self.node_connections:
                self.node_connections[node_from].append(node_to) # if node already has connections
            else:
                self.node_connections[node_from] = [node_to] # add new entry to dict
        elif node_to.returnName() == "pc": # check if node_to is a PC
            if node_to in self.node_connections:
                self.node_connections[node_to].append(node_from)
            else:
                self.node_connections[node_to] = [node_from]

        elif node_from.returnName() in ("switch", "hub"): # if no PC added, set bridging device as key
            if node_from in self.node_connections:
                self.node_connections[node_from].append(node_to)
            else:
                self.node_connections[node_from] = [node_to]
        else:  # if node_to is a bridging device
            if node_to in self.node_connections:
                self.node_connections[node_to].append(node_from)
            else:
                self.node_connections[node_to] = [node_from]
    ...
    def removeFromDict(self, Node): # remove route
        for key in self.node_connections.keys():
            if key == Node:
                del self.node_connections[key] # delete all cases of the node, where key = node

        for k, v in self.node_connections.items():  # k = key, v = values
            self.node_connections[k] = filter(lambda x: x != Node, v)  # filters every key's values to remove node

        for k, v in self.node_connections.items():  # checks for empty value lists
            if not v: # if key's value list is empty (has no connections) delete key
    ...
    def displayCables(self): # display cables between nodes
        keys = self.node_connections.keys() # get the dictionary keys
        for key in keys: # for each key in the matrix
            for value in self.node_connections[key]: # value list of the key
                x1, y1 = key.returnPosition()  # return position of nodeFrom
                x2, y2 = value.returnPosition()  # return position of nodeTo
                pygame.draw.aaline(screen, (0, 0, 255), (x1 + 20, y1 + 20), (x2 + 20, y2 + 20), 2)  # draw cable between nodes
...
class SpeedSlider:
    def __init__(self):
        ...
        self.pressed = False
        self.font = pygame.font.SysFont('Ariel', 22)  # initialise font type and size
            ...
            for ev in pygame.event.get():
                try:
                    if self.circle.collidepoint(ev.pos) or self.pressed:
                        ...
                        # ensure can't be dragged passed ends of the slider
                        if self.x < 1000:
                            self.x = 1000
                        elif self.x > 1130:
                            self.x = 1130
                        self.packet_speed = (130 - (self.x - 900)/2)  # calculate how much the movement of slider affects speed of node
                        screen.fill((250, 250, 250))
                        callBlitToScreen()
                        self.blitToScreen()
                        # update packet speed
                ...
                except AttributeError: # if no network has been created
                    pass
