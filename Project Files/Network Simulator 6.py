                        ...
                        if (x <= (coordinate[0]+511)) and ((x >= coordinate[0]+488)):
                            if (y <= (coordinate[1]+61)) and ((y >= coordinate[1]+38)):
                                ...
                                if selection in self.Properties:
                                    ...
                                    pygame.draw.rect(self.window_surface, (255, 255, 255), (coordinate[0]-12, coordinate[1]-12, 24, 24))
                                    pygame.draw.circle(self.window_surface, (0,0,0), (coordinate[0], coordinate[1]), 12, 1)
                                    for server in Server_drives:
                                        if server.returnActivated():
                                            server.RemoveReduntantCables()

                                else:
                                    self.window_surface.blit(pygame.image.load("tick.png"), (coordinate[0] - 9, coordinate[1] - 7))
...
class PC(Node):
    def __init__(self):
        ...
        self.PropertyConnections = [{"Word processing":"printer"},{"Internet browsing":"router"},{"Multimedia":"FILESERVER"}]
...
class Router(Node):
    def __init__(self):
        ...
        super(Router, self).__init__((10, 60), "router.png", "router_selected.png", "Router", 1, True, self.text1, self.CompatibleNodes)


class Switch(Node):
    def __init__(self):
        self.text1 = "Applications running:\n None"
        self.CompatibleNodes = ["pc", "router", "switch", "hub", "printer", "file server", "print server", "e-mail server", "multimedia server", "web cache"]
        super(Switch, self).__init__((10, 105), "switch.png", "switch_selected.png", "Switch", 1, True, self.text1, self.CompatibleNodes)
...
class Hub(Node):
    def __init__(self):
        self.text1 = "Applications running:\n None"
        self.CompatibleNodes = ["pc", "router", "switch", "hub", "printer", "file server", "print server", "e-mail server", "multimedia server", "web cache"]
        super(Hub, self).__init__((10, 170), "hub.png", "hub_selected.png", "Hub", 1, True, self.text1, self.CompatibleNodes)
...
class Printer(Node):
    def __init__(self):
        ...
        super(Printer, self).__init__((10, 215), "printer.png", "printer_selected.png", "Printer", 1, True, self.text1, self.CompatibleNodes)
...
class Server(Node):
    def __init__(self):
        ...
        super(Server, self).__init__((10, 265), "server.png", "server_selected.png", "Server", 1, False, self.text1, self.CompatibleNodes)
...
calculatePacketPath = CalculatePacketPath()
...
def runProgram():
    ...
    calculatePacketPath.CalculatePath()
    pygame.display.flip()
