...
class AdjacencyList(object):
    ...
    def AddToDict(self, NodeFrom, NodeTo, x1, y1):
        self.Activated = True
        if NodeFrom not in self.Node_Connections and NodeTo not in self.Node_Connections:
        ...
        elif NodeFrom in self.Node_Connections and NodeTo in self.Node_Connections:
            if NodeFrom.returnName() == "pc":
                self.Node_Connections[NodeFrom].append(NodeTo)
            else:
                self.Node_Connections[NodeTo].append(NodeFrom)

        elif NodeFrom in self.Node_Connections:
            if NodeTo.returnName() != "pc":
                self.Node_Connections[NodeFrom].append(NodeTo)
            elif NodeTo not in self.Node_Connections:
                self.Node_Connections[NodeTo] = [NodeFrom]
            else:
                self.Node_Connections[NodeTo].append(NodeFrom)
            ...
            if NodeFrom.returnName() != "pc":
                self.Node_Connections[NodeTo].append(NodeFrom)
            elif NodeFrom not in self.Node_Connections:
                self.Node_Connections[NodeFrom] = [NodeTo]
            else:
                self.Node_Connections[NodeFrom].append(NodeTo)
