...
class Simulation:
  ...
  def __CreateRiver(self):
    XCoordinate = self.__InputCoordinate("river x")
    YCoordinate = self.__InputCoordinate("river y")

    for y in range(YCoordinate, self.__LandscapeSize-YCoordinate):
      self.__Landscape[XCoordinate][y] = "R"
  ...
  def __DrawLandscape(self):
    ...
    for y in range (0, self.__LandscapeSize):
      ...
      print("", y, "|", sep = "", end = "")
      for x in range (0, self.__LandscapeSize):
        if not self.__Landscape[x][y].Warren is None or self.__Landscape[x][y] == "R":
